...
function useAbility(monster, monsterType) {
  ...
  switch(monsterType.id) {
    ...
    case 'flame-hunter':
      ...
      // Damage player if in flame barrier radius
      const playerPosition = camera.position.clone();
      const monsterPosition = monster.position.clone();
      const distance = playerPosition.distanceTo(monsterPosition);
      
      if (distance < 5) {
        // Simulate damage to player
        console.log('Player damaged by flame barrier!');
        // You can add actual damage logic here
      }
      break;
...
function resetAbilityEffects(monster, monsterType) {
  switch(monsterType.id) {
    ...
    case 'flame-hunter':
      ...
      // Remove flame spikes
      if (monster.userData.flameSpikeGroup) {
        scene.remove(monster.userData.flameSpikeGroup);
        monster.userData.flameSpikeGroup = null;
      }
      break;
  }
}

function createFlameSpikes(monster) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    // Randomize spike position in the maze
    let validPosition = false;
    let attempts = 0;
    ...
    spike.userData.animate = () => {
      ...
      spike.position.y = 0.5 + Math.sin(time * 2 + i) * 0.2;
      spike.rotation.y += 0.01;
      spike.material.opacity = 0.8 + Math.sin(time * 3) * 0.2;
    };

    spikes.add(spike);
  }

  monster.userData.flameSpikeGroup = spikes;
  scene.add(spikes);
}
...
function animate() {
  ...
  if (!gameOver && !won && !isPaused && controls.isLocked) {
    ...
    if (monster) {
      ...
      // Animate flame spikes
      if (monster.userData.flameSpikeGroup) {
        monster.userData.flameSpikeGroup.children.forEach(spike => {
